...
def create_on_device(module_class: Type[nn.Module], device: Optional[torch.device], *args, **kwargs) -> nn.Module:
    """ Create new instance on the device if its constructor supports this.
    :param module_class:  should be nn.Module
    :param device:  torch device
    :param args:  args passed to nn.Module constructor.
    :param kwargs:  kwardgs passed to nn.Module constructor.
    :return:
    """
    arg_spec = inspect.getfullargspec(module_class.__init__)
    if "device" in arg_spec.args or "device" in arg_spec.kwonlyargs:
