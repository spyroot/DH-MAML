...
class ConjugateGradientOptimizer(Optimizer):
    ...
    def __setstate__(self, state):
        ...
        if 'hvp_reg_coeff' not in state['state']:
            warnings.warn('Resuming ConjugateGradientOptimizer with lost state. '
    ...
    def _backtracking_line_search(self, params, descent_step, f_loss, f_constraint):
        ...
        if ((torch.isnan(loss) or torch.isnan(constraint_val)
            ...
            warnings.warn('Line search condition violated. Rejecting the step!')
            if torch.isnan(loss):
                warnings.warn('Violated because loss is NaN')
            if torch.isnan(constraint_val):
                warnings.warn('Violated because constraint is NaN')
            if loss >= loss_before:
                warnings.warn('Violated because loss not improving')
            if constraint_val >= self._max_constraint_value:
                warnings.warn('Violated because constraint is violated')
