...
class Memory(object):
    """
    Memory  storing (context, action, reward) transitions.
    Attributes:
    ...
    def __init__(self, device: Union[str, torch.device] = "cpu"):
        ...
        if type(device) is str:
            self.device = (torch.device(device)
                if "cuda" in device and torch.cuda.is_available()else torch.device("cpu"))
        else:
            self.device = device
    ...
    def get_data(self, batch_size: Union[int, None] = None) -> \
        ...
        x = (torch.stack([self.db["contexts"][i] for i in idx]).to(self.device).to(torch.float))
        y = (torch.tensor([self.db["rewards"][i] for i in idx]).to(self.device).to(torch.float).unsqueeze(1))
        a = (torch.stack([self.db["actions"][i] for i in idx]).to(self.device).to(torch.long))
        return x, a, y

    def get_data_for_action(self, action: int, batch_size: Union[int, None] = None) -> Tuple[torch.Tensor, torch.Tensor]:
